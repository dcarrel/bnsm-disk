...
/* ------------------------------------------------------
  ...
  printf("Writing coords for rank %03d to file %s\n", prank, &cfname);
  PLUTO_coords = fopen(cfname_buf, "w");
  ITOT_LOOP(i){
    x1 = grid->x[IDIR][i];
    fprintf(PLUTO_coords,"%f\t", x1);
  }
  fprintf(PLUTO_coords, "\n");
  JTOT_LOOP(j){
    x2 = grid->x[JDIR][j];
    fprintf(PLUTO_coords, "%f\t",x2);
  }
  fclose(PLUTO_coords);
  // system call to generate initial data for this grid
  char syscall_buff[0x100];
  snprintf(syscall_buff, sizeof(syscall_buff), "python initgridn.py %i", prank);
  system(syscall_buff);
  ...
  printf("reading initial data for rank %03d from file %s\n",prank, &ifname);
  init_file = fopen(ifname_buf, "r");
  KTOT_LOOP(k) { 
  JTOT_LOOP(j) { 
  ITOT_LOOP(i) { 
  ...
  /* -- Reset arrays -- */
    ...
    #else
    fscanf(init_file, "%lf\t%lf\t%lf\t%lf\t%lf\n",
                       &u_av[RHO], &u_av[PRS], &u_av[VX1], &u_av[VX2], &u_av[VX3]);
    Init (u_av, x1, x2, x3);
