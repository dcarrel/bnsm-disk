 ...
 *********************************************************************** */
{
  ...
  if (side == X1_END){  /* -- X1_END boundary -- */
    if (box->vpos == CENTER) {
      // Deletes all radially inwards velocities (i.e. no inward massflux)
      BOX_LOOP(box,k,j,i){
  ...
  if (side == X2_BEG){  /* -- X2_BEG boundary -- */
    if (box->vpos == CENTER) {
      //gets rid of inflow, polaraxis is really bad at resolving shocks
      BOX_LOOP(box,k,j,i){
        double vx2 = d->Vc[VX2][k][JBEG][i];
	if (vx2 > 0.){
	  d->Vc[VX2][k][j][i] = 0.0;
	}
	else{
 ...
 *********************************************************************** */
{
  return -UNIT_G*g_inputParam[MBH]/x1;
}
#endif
