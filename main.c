...
static int SetMaxPrim (int, Data *);
...
// Loop over grid and set global variable MaxPrim every timestep
int SetMaxPrim(int PRIM, Data *d){
  int i,j,k;
  double prim_curr;
  DOM_LOOP (k,j,i){
    prim_curr = d->Vc[PRIM][k][j][i];
    if (prim_curr > g_maxPrim){
      g_maxPrim = prim_curr;
    }
  }
  return 0;
