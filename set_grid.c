...
/*    fprintf (fg, "# %s\n", getenv("PWD")); */
    ...
    fclose(fg);
    //print out pcoord file for loading later
    printf("deleting all old datafiles\n");
    system("rm *.dat");
    FILE * PCOORDS;
    PCOORDS = fopen("pgrid4py.dat", "w");
    for (idim=0; idim < 2; idim++){
      ngh = grid->nghost[idim];
      iend = grid->gend[idim];
      ibeg = grid->gbeg[idim];
      for(i=ibeg-ngh; i <= iend + ngh; i++){
        fprintf(PCOORDS, "%f\t", grid->x_glob[idim][i]);
     }
     fprintf(PCOORDS,"\n");
    }
    fclose(PCOORDS);
    //generates initial data for python
    printf("Writing initial grid file...\n");
    system("python initgrid.py");
  }
